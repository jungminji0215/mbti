...
import { useMutation, useQueryClient } from "@tanstack/react-query"; // tanstack의 useMutation과 useQueryClient import
...
const TestResultItem = ({ testResult }) => {
  ...
  const isOwner = testResult.userId === user.id;

  const formattedDate = new Date(testResult.date).toLocaleString();

  const description = mbtiDescriptions[testResult.mbti];
  ...
  const handleDelete = () => {
    mutate(testResult.id);
  };

  const visibilityMutation = useMutation({
    mutationFn: (newVisibility) =>
      updateTestResultVisibility(testResult.id, newVisibility),
    onSuccess: () => {
      queryClient.invalidateQueries(["testResults"]);
    },
    onError: (error) => {
  ...
  const handleToggleVisibility = () => {
    const newVisibility = !testResult.visibility;
